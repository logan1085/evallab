...
// 2. Cases written; the panel offered.
const body0 = await page.textContent('body');
ok('six cases in standfirst', body0.includes('6 cases'));
ok('placeholder notice without a key', body0.includes('Placeholder scenarios'));
...
ok('seats labeled simulated without keys', body1.includes('simulated'));
...
await page.waitForSelector('text=Simulated panel', { timeout: 60000 });
await page.waitForTimeout(600);
const mapText = await page.textContent('body');
ok('map arrived after the run', /split on \d+ of 6 cases|agreed on everything/.test(mapText));
ok('simulated verdicts say so', mapText.includes('Simulated panel'));
