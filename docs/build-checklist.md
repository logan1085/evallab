...
- [x] Checked-in pin registry: pin_id, family, explicit versioned model id,
      provider_slug, tier, status; Ploom-compatible field names
- [x] One `callModel()` gateway: caller tags, provider.only single slug,
...
- [x] BYOK: the key arrives per request, never module-global, never persisted
      or logged; a test proves absence from every recorded object
- [x] scripts/model-smoke.ts
...
- [x] model_call table: one row per attempt incl. retries/failures, usage read
      off the router, generation_id, indexed by round and seat
- [x] costForRound / costForPanelist aggregates; nothing rolled up
- [x] Spend guard: per-round + daily ceilings from env, typed budget_exceeded,
...
- [x] Required reason enforced as schema failure, not defaulted
- [x] 20% repeat sample per seat → self_consistency; below-threshold seats
      flagged and down-weighted
- [x] output_length recorded per verdict; shuffle seed stored
- [x] Pinned model-version map stored on the round row
- [x] Running cost on the status endpoint, summed from model_call
...
- [x] AC1 variance estimator (Gwet 2008), cited
- [x] Length-vs-pass correlation per seat
- [x] Typed undefined results: { value: null, reason } surfaced to the UI

## Sitting 7: the disagreement map

- [x] Four patterns; settled provisional until checked; dissenter named;
      ranked sections
- [x] The literalist test: persona-driven splits the literalist does not
...
- [x] Projected lift as a real recomputation of the agreement statistic under
      the amended outcome, not a coverage ratio
- [x] panel_edit rows as candidate clauses (the deleted-seat signal)
- [x] Changelog on the new version naming patches and source round
...
- [x] False-settle rate recorded on the round
- [x] Who-speaks-for-you benchmarked against the 81% human ceiling in copy
- [x] Re-weighting the panel toward the seats that share the user's taste
- [x] A false settle convertible to a rubric patch in one click
...
- [x] Round cost (total + per seat) in the bundle
- [x] Content hash per artifact, recorded in an export table
- [x] Re-run comparison: flip list with old verdict, new verdict
- [x] scripts/recovery-test.ts: delete a clause, re-run, check the patch
